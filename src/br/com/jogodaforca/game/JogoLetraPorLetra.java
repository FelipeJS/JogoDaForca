...
public class JogoLetraPorLetra implements JogoStrategy, Observer {
	...
	public void jogar(Palavra palavra) {
		...
		Map<String, Integer> mapaLetras = new HashMap<String, Integer>();
			...
			while (continuarJogando) {
				...
				for (int tam = 0; tam < palavra.getPalavra().length(); tam++) {
					if (letra.getLetra() == palavra.getPalavra().charAt(tam)
							&& errou == true) {
						quantLetras++;
						if (!mapaLetras.containsKey(String.valueOf(letra
								.getLetra()))) {
							palavraDinamica[tam] = letra.getLetra();
							errou = false;
							mapaLetras.put(String.valueOf(letra.getLetra()),
									quantLetras);
						}

						else {
							if (mapaLetras
									.get(String.valueOf(letra.getLetra()))
									.intValue() < quantLetras) {
								palavraDinamica[tam] = letra.getLetra();
								errou = false;
								mapaLetras.put(
										String.valueOf(letra.getLetra()),
										quantLetras);
							}
						}
